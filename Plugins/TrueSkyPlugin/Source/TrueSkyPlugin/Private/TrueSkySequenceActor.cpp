...
const FName ATrueSkySequenceActor::kMPC_ShadowEdgeSmoothness = TEXT("ShadowEdgeSmoothness");
const FName ATrueSkySequenceActor::kMPC_SunlightTableTransform = TEXT("SunlightTableTransform");
const FName ATrueSkySequenceActor::kMPC_CloudTint = TEXT("CloudTint");
...
ATrueSkySequenceActor::ATrueSkySequenceActor(const class FObjectInitializer& ObjectInitializer)
	...
	, StarLightIntensity(0.05f)
	, StarLightCastsShadows(true)
	, StarLightColor(1.0f,1.0f,1.0f,1.0f)
	, SkyMultiplier(1.0f)
	, SunMultiplier(1.0f)
	, MoonMultiplier(1.0f)
	, Brightness(1.0f)
	...
	,MoonsMeshesChecksum(0)
{
	...
	//OverlayRTAssetRef = kDefaultOverlayRT;  Temp Disable while bug Fixed Regarding UI issues
	TranslucentRTAssetRef = kDefaultTranslucentRT;
	MaterialParameterCollectionAssetRef = kDefaultMaterialParameterCollection;
	CloudTint=FLinearColor::White;
	MoonLightingChannel=1;
...
#if SIMUL_ALTITUDE_DIRECTIONAL_LIGHT
	bAltitudeDirectionalLight=true;
#else
	bAltitudeDirectionalLight=false;
#endif
}
	
void ATrueSkySequenceActor::UpdateMoonPositions()
{
	ActorCrossThreadProperties *A	=GetActorCrossThreadProperties();
	static float dist=10000000.0f;
	static FVector offset;
	TArray<UStaticMeshComponent*> gotMeshes;
	GetComponents<UStaticMeshComponent>(gotMeshes);	
	if(LightForMoons)
	{
		FVector SunDirection=GetVectorProperty(ETrueSkyPropertiesVector::TSPROPVECTOR_SunDirection);
		float azimuth			=180.0f/3.1415926536f*atan2(SunDirection.X,SunDirection.Y);
		float elevation			=180.0f/3.1415926536f*asin(SunDirection.Z);
		//LightForMoons->SetCastShadows(true);
		//LightForMoons->SetIntensity(StarLightIntensity);
		//linearColour.A=1.0;
		//DirectionalLightComponent->SetLightColor(linearColour,false);
		UDirectionalLightComponent* DirectionalLightComponent = CastChecked<UDirectionalLightComponent>(LightForMoons->GetLightComponent());
	...
	{														 	
		FMoon &moon=Moons[i];
		// use id=index+1				 	
		if(A->Moons.Contains(i+1))
		{
			auto &ProxyMoon=A->Moons.FindOrAdd(i+1);
			moon.Direction=ProxyMoon.Direction;
			moon.Orientation=ProxyMoon.Orientation;
		}
	}
	for(auto &M:Moons)
	{
		if(M.CurrentMeshComponent)
		{
			offset	=dist*TrueSkyToUEDirection(A->Transform,M.Direction);
			M.CurrentMeshComponent->SetRelativeLocation(offset);
			const float pi=3.1415926536f;
			// angular size of an object of width D at distance d is 2*atan2(D/2d)
			// desired angular size is size_radians=M.RadiusArcMinutes*pi/180.0f/60.0f radians.
			// The object is actually of size M, so at distance d it has angular size 2*atan2(M*scale/2d).
			// Therefore if radius_radians=atan2(M*scale,2d),
			float radius_radians=M.RadiusArcMinutes*pi/180.0f/60.0f;
			// Say object is 1 metre in width:
			static float size_cm=100.0f;
			float scale=tan(radius_radians)*2.f*dist/size_cm;
			FVector newscale=scale*FVector(1.f,1.f,1.f);
			M.CurrentMeshComponent->SetRelativeScale3D(newscale);
			if(!gotMeshes.Contains(M.CurrentMeshComponent))
			{
				continue;
			}
		}
		else if(M.StaticMesh)
		{
			UpdateMoonMeshes();
		}
	}
...
void ATrueSkySequenceActor::UpdateMoonMeshes()
{
	TArray<UStaticMeshComponent*> requiredMeshes;
	TArray<UStaticMeshComponent*> gotMeshes;
	GetComponents<UStaticMeshComponent>(gotMeshes);
	for(auto &M:Moons)
	{
		if(M.StaticMesh)
		{
			// Does it already have one, and is it present?
			if(M.CurrentMeshComponent)
			{
				// We already have the component for this.
				if(gotMeshes.Contains(M.CurrentMeshComponent))
				{
					M.CurrentMeshComponent->SetStaticMesh(M.StaticMesh); 
					if(M.MeshMaterial)
						M.CurrentMeshComponent->SetMaterial(0,M.MeshMaterial);
					M.Render=false;
					continue;
				}
				M.CurrentMeshComponent=nullptr;
			}
			if(!M.CurrentMeshComponent)
			{
				// We must create a mesh that corresponds.
				//M.CurrentMeshComponent = CreateDefaultSubobject<UStaticMeshComponent>(FName(*M.Name));
				// CreateDefaultSubObject only works in constructors. instead:

				
				// Construct an object of the class passed to us but return a point to USceneComponent  
				M.CurrentMeshComponent = NewObject<UStaticMeshComponent>(this, FName( *M.Name ));
				M.CurrentMeshComponent->AttachToComponent(RootComponent, FAttachmentTransformRules::KeepWorldTransform);
				M.CurrentMeshComponent->SetStaticMesh(M.StaticMesh); 
				M.CurrentMeshComponent->RegisterComponent(); 
				//Attach the component and position it at the location given  
				//M.CurrentMeshComponent->SetupAttachment(RootComponent); 
				M.CurrentMeshComponent->SetRelativeLocation(FVector(0.f, 0.f, 0.f));
				M.CurrentMeshComponent->LightingChannels.bChannel0=(MoonLightingChannel==0);
				M.CurrentMeshComponent->LightingChannels.bChannel1=(MoonLightingChannel==1);
				M.CurrentMeshComponent->LightingChannels.bChannel2=(MoonLightingChannel==2);
				if(M.MeshMaterial)
					M.CurrentMeshComponent->SetMaterial(0,M.MeshMaterial);
			}
			if(M.CurrentMeshComponent)
			{
				gotMeshes.Remove(M.CurrentMeshComponent);
				M.Render=false;
			}
			else
				M.Render=true;
	...
	// now delete any meshes that are not used.
	for(auto &m:gotMeshes)
	{
...
void ATrueSkySequenceActor::PostInitProperties()
{
	...
	// So we MUST NOT call TransferPropertiesToProxy here, or else we will get GPU memory etc allocated with the wrong values.
	//TransferPropertiesToProxy();
...
void ATrueSkySequenceActor::UpdateTime()
{
	ActorCrossThreadProperties* A = GetActorCrossThreadProperties();
		...
		if (Time > LoopEnd)
			Time = LoopStart;
		else if (Time < LoopStart)
			Time = LoopStart;
		}

	if (A->GetTime)
	{
		Time = A->Time * TimeUnits;
		A->GetTime = false;
	}
	else if((bPlaying&&TimeProgressionScale !=0) || (Time/TimeUnits) != A->Time)
	{
		//Allowing for personalised units of time (Day is 0-1, 0-24 or 0-100 etc.)
		Time += (((TimeProgressionScale / float(24*60*60)) * TimeUnits) * GetWorld()->GetDeltaSeconds());
		A->Time = Time / TimeUnits;
		A->SetTime = true;
		UpdateMoonPositions();
	}
}
...
void ATrueSkySequenceActor::SetDefaultValues()
{
	//can be expanded upon

	SimulVersion simulVersion = GetTrueSkyPlugin()->GetVersion();
	bSimulVersion4_1 = (simulVersion >= SIMUL_4_1);
...
{
	//ActorCrossThreadProperties* A = GetActorCrossThreadProperties();
	Time=value;//We could remove this function in the future and make all changes apply to the variable.
...
int32 ATrueSkySequenceActor::GetInterpolatedCloudKeyframe(int32 layerUID)
{
	TArray<FVariant> arr;
	arr.Push(FVariant(layerUID));
...
bool ATrueSkySequenceActor::UpdateLight(ADirectionalLight* DirectionalLight,float sun_multiplier,float moon_multiplier,bool include_shadow,bool apply_rotation, bool light_in_space, bool StarLightCastsShadows, float StarLightIntensity, FLinearColor StarLightColor)
{
	ActorCrossThreadProperties *A=GetActorCrossThreadProperties();
	if(!A)
		return false;
	...
	if(!DirectionalLightComponent)
		return false;
	return( UpdateLightComponent(DirectionalLightComponent, sun_multiplier,moon_multiplier, include_shadow, apply_rotation, light_in_space, StarLightCastsShadows, StarLightIntensity, StarLightColor) );
}
...
bool ATrueSkySequenceActor::UpdateLightComponent(UDirectionalLightComponent* DirectionalLightComponent,float sun_multiplier,float moon_multiplier,bool include_shadow
	,bool apply_rotation,bool light_in_space, bool StarLightCastsShadows, float StarLightIntensity, FLinearColor StarLightColor)
{
	ActorCrossThreadProperties* A = GetActorCrossThreadProperties();
	...
	if (res.valid)
	{
		...
		else
		{
			...
			{
				DirectionalLightComponent->SetCastShadows(StarLightCastsShadows);
				DirectionalLightComponent->SetIntensity(StarLightIntensity);
				linearColour= StarLightColor;
				azimuth = 0.f;
				elevation = 90.f;
			}
			if (l < StarLightIntensity || m == 0.0f)
				DirectionalLightComponent->SetIntensity(StarLightIntensity);
		}
		linearColour.A=1.0;
		DirectionalLightComponent->SetLightColor(linearColour,false);
...
void ATrueSkySequenceActor::TransferPropertiesToProxy()
{
	ActorCrossThreadProperties *A	=GetActorCrossThreadProperties();
	if(!A)
		return;
	...
	//temp changes while we re-work transferring of values
	if (A->InterpolationSubdivisions != InterpolationSubdivisions)
	{
		A->InterpolationSubdivisions = InterpolationSubdivisions;
		SetInteger(ETrueSkyPropertiesInt::TSPROPINT_InterpolationSubdivisions, A->InterpolationSubdivisions); //This is being hard coded when other interpolation values are set
	}

	if (A->InterpolationMode != (uint8_t)InterpolationMode)
	{
		A->InterpolationMode = (uint8_t)InterpolationMode;
		SetInteger(ETrueSkyPropertiesInt::TSPROPINT_InterpolationMode, (int)A->InterpolationMode);
	}

	if (A->InterpolationTimeSeconds != InterpolationTimeSeconds)
	{
		A->InterpolationTimeSeconds = InterpolationTimeSeconds;
		SetFloatProperty(ETrueSkyPropertiesFloat::TSPROPFLOAT_InterpolationTimeSeconds, A->InterpolationTimeSeconds);
	}
	if (A->InterpolationTimeHours != InterpolationTimeHours)
	{
		A->InterpolationTimeHours = InterpolationTimeHours;
		SetFloatProperty(ETrueSkyPropertiesFloat::TSPROPFLOAT_InterpolationTimeDays, A->InterpolationTimeHours / 24.0f);
	}
	...
	A->Gamma						=Gamma;
	UWorld *world					=GetWorld();
	A->IntegrationScheme			=(unsigned char)IntegrationScheme;
	...
	if (CloudShadowRT && (CloudShadowRT->SizeX != ShadowTextureSize))
	{
		CloudShadowRT->SizeX = ShadowTextureSize;
		CloudShadowRT->SizeY = ShadowTextureSize;
	}
	...
	if(MoonsMeshesChecksum!=MoonsChecksum)
	{
		UpdateMoonMeshes();
		MoonsMeshesChecksum=MoonsChecksum;
	}
	if(MoonsChecksum!=A->MoonsChecksum)
	{
		...
		for(int i=0;i<Moons.Num();i++)
		{
			...
			// use id=index+1
			auto &ProxyMoon=A->Moons.FindOrAdd(i+1);
			ProxyMoon.Name=moon.Name;
			ProxyMoon.orbit={moon.LongitudeOfAscendingNode
			...
			ProxyMoon.Render=moon.Render;
			ProxyMoon.RadiusArcMinutes=moon.RadiusArcMinutes;
			ProxyMoon.Texture=GetTextureRHIRef(moon.Texture);	
			ProxyMoon.Direction=FVector(0,0,-1.0f);
		}													 	
	...
	SetVectorProperty(ETrueSkyPropertiesVector::TSPROPVECTOR_Render_CloudTint,(FVector)CloudTint);
	...
	//for(int i=0;i<4;i++) //we loop here while LightningBolts only stores 1 value.
	{
		simul::LightningBolt *L=&(A->lightningBolts[0]);
...
void ATrueSkySequenceActor::FillMaterialParameters( )
{
	...
	if( TrueSkyPropertyFloatMaps.Num( ) )
	{
		...
		UKismetMaterialLibrary::SetScalarParameterValue(this, TrueSkyMaterialParameterCollection, kMPC_ShadowEdgeSmoothness, ShadowEdgeSmoothness);
...
void ATrueSkySequenceActor::TickActor(float DeltaTime,enum ELevelTick TickType,FActorTickFunction& ThisTickFunction)
{
...
#endif
		UpdateLight((ADirectionalLight*)Light,SunMultiplier,MoonMultiplier,false,DriveLightDirection, AltitudeLighting, StarLightCastsShadows, StarLightIntensity, StarLightColor);
	}
	UWorld *world = GetWorld();
	...
	// Find out which trueSKY actor should be active. We should only do this once per frame, so this is inefficient at present. Functionality for different actors per world has been removed.
...
void ATrueSkySequenceActor::ClearRenderTargets()
{
	UKismetRenderingLibrary::ClearRenderTarget2D(this, TranslucentRT);
	UKismetRenderingLibrary::ClearRenderTarget2D(this, OverlayRT);
	UKismetRenderingLibrary::ClearRenderTarget2D(this, LossRT);
	UKismetRenderingLibrary::ClearRenderTarget2D(this, CloudVisibilityRT);
	UKismetRenderingLibrary::ClearRenderTarget2D(this, CloudShadowRT);
	UKismetRenderingLibrary::ClearRenderTarget2D(this, AltitudeLightRT);
	UKismetRenderingLibrary::ClearRenderTarget2D(this, InscatterRT);
...
{
	// for some reason, UE Unregisters ALL components on edit.
	for(auto &M:Moons)
	{
		if(M.CurrentMeshComponent)
			M.CurrentMeshComponent->RegisterComponent();
	...
	if(!PropertyChangedEvent.MemberProperty)
		return;
	if(PropertyChangedEvent.MemberProperty->GetName().Contains("Moon"))
		MoonsChecksum++;
