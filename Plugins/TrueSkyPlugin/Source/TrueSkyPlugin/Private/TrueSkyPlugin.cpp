...
#if PLATFORM_WINDOWS || PLATFORM_XBOXONE || PLATFORM_PS4 || PLATFORM_UWP || PLATFORM_SWITCH
#define TRUESKY_PLATFORM_SUPPORTED 1
#else
    #if PLATFORM_LINUX && SIMUL_SUPPORT_VULKAN
#define TRUESKY_PLATFORM_SUPPORTED 1
#else
...
{
	trueSkyPluginPath=IPluginManager::Get().FindPlugin(TEXT("TrueSkyPlugin"))->GetBaseDir();
...
void FTrueSkyPlugin::SetVector(ETrueSkyPropertiesVector Property, FVector Value)
{
	...
	{
		auto &propMap=ATrueSkySequenceActor::TrueSkyPropertyVectorMaps[ATrueSkySequenceActor::TSPropertiesVectorIdx];
		if(propMap.Contains((int64)Property))
		{
			auto p = propMap.Find((int64)Property);
		if(((p->TrueSkyEnum > 0) && (!(p->PropertyValue - Value).IsNearlyZero())) || !p->Initialized)
		{
			...
			{
				UE_LOG_ONCE(TrueSky, Warning, TEXT("Trying to set a vector property %d but it was not initialized!"),(int64)Property);
			}
		}
		else
		{
			UE_LOG_ONCE(TrueSky, Warning, TEXT("Trying to set a vector property %d but it was not found!"),(int64)Property);
		}
	}
	else
	{
		UE_LOG_ONCE(TrueSky, Warning, TEXT("Trying to set a vector property %d but the index is out of range or the array is empty!"),(int64)Property);
	}
}
...
void FTrueSkyPlugin::SetFloat(ETrueSkyPropertiesFloat Property, float Value)
{
	...
	{
		auto &propMap=ATrueSkySequenceActor::TrueSkyPropertyFloatMaps[ATrueSkySequenceActor::TSPropertiesFloatIdx];
		if(propMap.Contains((int64)Property))
		{
			auto p = propMap.Find((int64)Property);
		if (((p->TrueSkyEnum > 0) && (!FMath::IsNearlyZero(p->PropertyValue - Value)))||!p->Initialized)
		{
			...
			{
				UE_LOG_ONCE(TrueSky, Warning, TEXT("Trying to set a float property %d but it was not initialized!"),(int64)Property);
			}
		}
		else
		{
			UE_LOG_ONCE(TrueSky, Warning, TEXT("Trying to set a float property %d but it was not found!"),(int64)Property);
		}
	}
	else
	{
		UE_LOG_ONCE(TrueSky, Warning, TEXT("Trying to set a float property %d but the index is out of range or the array is empty!"),(int64)Property);
	}
}
				...
				, bool isTranslucentPass) const
{
	...
	if(!isTranslucentPass)
		f.colour.resourceState=(unsigned int)simul::crossplatform::ResourceState::RENDER_TARGET;
...
void FTrueSkyPlugin::SetRenderTexture(FRHICommandListImmediate *RHICmdList,const char *name, FRHITexture* texture)
{
	if(!RHICmdList)
		return;
	IRHICommandContext& RHICommandContext=RHICmdList->GetContext();
	IRHICommandContext* pRHICommandContext=&RHICommandContext;
	if(!pRHICommandContext)
	{
		UE_LOG(TrueSky,Error, TEXT("Null command context - this should never happen!"));
		IRHICommandContext& RHICommandContext2=RHICmdList->GetContext();
		pRHICommandContext=&RHICommandContext2;
		if(!pRHICommandContext)
			return;
	}
	...
	if (StaticSetRenderTexture2)
	{
		...
		{
			InitExternalTexture(pRHICommandContext,ext, texture, CurrentRHIType);
			ext.resourceState=(int)GetTextureResourceState(pRHICommandContext,texture,CurrentRHIType);
		}
		else
		{
														...
														,bool clear)
{
	...
	FRHITexture2D *TargetTexture= RenderParameters.RenderTargetTexture;
	void* colourTargetTexture = GetPlatformTexturePtr(TargetTexture, CurrentRHIType);
...
void FTrueSkyPlugin::DelegatedRenderFrame(FRenderDelegateParameters& RenderParameters)
{
	...
	else
	{
		...
		{
			// TODO: verify splitscreen support: "LastTaskCompletionEvent" should be enough to sync (serialize) all opaque and translucent passes
			//	  and views, but, if more parallelism is required, add more completion events to sync them.
			// TODO: maybe UpdateFromActor() (in RenderFrame) should be called here and moved away from async task.
			// TODO: all "renderingenabled" checks, done by RenderFrame, could be moved here, in order to avoid useless task calls.
														...
														,int FrameNumber)
{
	...
	// Post-trans
	if (actorCrossThreadProperties&&actorCrossThreadProperties->TranslucentRT.GetReference()&&PostTranslucentEnabled)
	{
		RenderPostTranslucent(uid, RenderParameters, StereoPass, bMultiResEnabled, FrameNumber, true);
	}

	if(actorCrossThreadProperties&&actorCrossThreadProperties->OverlayRT.GetReference()&&OverlaysEnabled)
...
bool FTrueSkyPlugin::Exec( UWorld* , const TCHAR* Cmd, FOutputDevice& Out )
{
...
#endif
#if UE_BUILD_DEBUG// UE_BUILD_DEBUG //doesn't work... why?
...
bool FTrueSkyPlugin::EnsureRendererIsInitialized()
{
	...
	if(!RendererInitialized)
	{
		...
		if( device != nullptr )
		{
			FString PluginBasePath = IPluginManager::Get().FindPlugin(TEXT("TrueSkyPlugin"))->GetBaseDir();
			FString PluginAlternatePath;
#if PLATFORM_PS4
			PluginAlternatePath="/app0/engine/plugins/trueskyplugin";
#endif
			FString EnginePath=FPaths::EngineDir();
			StaticSetFileLoader(&ue4SimulFileLoader);
			FString shaderbin(FPaths::Combine(PluginBasePath, TEXT("/shaderbin/"), SimulUnrealPlatformName));
			FString texpath(FPaths::Combine(PluginBasePath, TEXT("/resources/media/textures/")));
			StaticPushPath("TexturePath", FStringToUtf8(texpath).c_str());
			FString texpath2(FPaths::Combine(PluginBasePath, TEXT("/textures")));
			StaticPushPath("TexturePath", FStringToUtf8(texpath2).c_str());

			if(PluginAlternatePath.Len())
			{
				StaticPushPath("TexturePath", FStringToUtf8(FPaths::Combine(PluginAlternatePath, TEXT("/textures"))).c_str());
				StaticPushPath("ShaderBinaryPath", FStringToUtf8(FPaths::Combine(PluginAlternatePath, TEXT("/shaderbin/"), SimulUnrealPlatformName)).c_str());
			}

#if PLATFORM_LINUX
            FString texpath3(FPaths::Combine(PluginBasePath, TEXT("/resources/media/textures")));
            StaticPushPath("TexturePath", FStringToUtf8(texpath3).c_str());
            FString texpath4(FPaths::Combine(PluginBasePath, TEXT("/resources/media/textures")));
            StaticPushPath("TexturePath", FStringToUtf8(texpath4).c_str());
#endif
#if PLATFORM_WINDOWS || PLATFORM_UWP
			if(haveEditor)
			{
				StaticPushPath("ShaderPath",FStringToUtf8(FPaths::Combine(PluginBasePath, TEXT("/Resources/Platform/DirectX11/HLSL"))).c_str());
				StaticPushPath("TexturePath",FStringToUtf8(FPaths::Combine(PluginBasePath, TEXT("/textures"))).c_str());
			}
			else
			{
				...
				StaticPushPath("TexturePath",FStringToUtf8(gamePath+L"\\content\\trueskyplugin\\textures").c_str());
			}
#endif
			FString shaderBinFull;
			if (CurrentRHIType == GraphicsDeviceVulkan)
			{
				shaderbin += "/Vulkan";
			}
			else if (CurrentRHIType == GraphicsDeviceD3D11)
			{
				shaderBinFull = IFileManager::Get().ConvertToAbsolutePathForExternalAppForRead(*shaderbin);
...
void FTrueSkyPlugin::UpdateFromActor(FTrueSkyRenderDelegateParameters& RenderParameters)
{
	...
	if(RenderingEnabled&&RendererInitialized)
	{
		...
		SetInt(ETrueSkyPropertiesInt::TSPROPINT_GridRendering, (int)actorCrossThreadProperties->IntegrationScheme);
		...
		if(StaticSetMoon)
		{
			for (auto &q : actorCrossThreadProperties->Moons)
			{
				...
				else
				{
					...
					moon.render=Moon.Render;
					std::string name=FStringToUtf8(Moon.Name);
					moon.name=name.c_str();
					StaticSetMoon(q.Key,&moon);
					// And receive the direction:
					Moon.Direction.X=moon.direction.x;
					Moon.Direction.Y=moon.direction.y;
					Moon.Direction.Z=moon.direction.z;
					Moon.Orientation.X=moon.orientation.x;
					Moon.Orientation.Y=moon.orientation.y;
					Moon.Orientation.Z=moon.orientation.z;
					Moon.Orientation.W=moon.orientation.w;
				}
			}
		}
