...
class TRUESKYPLUGIN_API UTrueSkyLightComponent : public USkyLightComponent
{
	...
	/** How often to update. 1 means every frame, 2 every other frame, etc. */
	...
	float Blend;
	/** How much to amortize rendering (low values fill more texels, performance may suffer)*/
	UPROPERTY(EditAnywhere,BlueprintReadWrite, Category=TrueSky,meta=(ClampMin = "1", ClampMax = "8"))
	int32 Amortization;
	/** Whether to fill all cubemap faces on every update (performance may suffer)*/
	UPROPERTY(EditAnywhere,BlueprintReadWrite, Category=TrueSky)
	bool AllFaces;
	/** Whether to fill all mipmaps on every update (performance may suffer)*/
	UPROPERTY(EditAnywhere,BlueprintReadWrite, Category=TrueSky)
	bool AllMips;
	
	/** A multiplier for diffuse illumination from the skylight. 1.0 is correct for physically-based rendering. */
	UPROPERTY(EditAnywhere,BlueprintReadWrite, Category=TrueSky, meta=(DisplayName = "Diffuse Multiplier", UIMin = "0.0", UIMax = "20.0"))
	float DiffuseMultiplier;
	
	/** A multiplier for specular reflections from the skylight. 1.0 is correct for physically-based rendering. */
	...
	/** A gamma adjustment to apply. Only 1.0 is strictly correct for physically-based rendering. */
	UPROPERTY(EditAnywhere,BlueprintReadWrite, Category=TrueSky, meta=(DisplayName = "Gamma Correction", UIMin = "0.01", UIMax = "20.0"))
	float Gamma;
	...
	void UpdateEnlighten(int face,int x,int y,int w,int h,FLinearColor *in_colours);
	/** 
