...
public:
	...
	/**Enabling this setting means that you can adjust the time through the Time variable below, and time will progress, even when not simulating. This will disable adjusting the time using the sequencer at runtime.*/
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Time)
	bool ControlTimeThroughActor;

	/**Current time. Uses decimals (16.5 = 16:30)*/
	...
	UPROPERTY(Interp, EditAnywhere, BlueprintReadWrite, Category = Time, meta = (UIMin = "0", UIMax = "3600"))
	float TimeFactor;

	/**Current time used by trueSKY. One day is 0 to 1. So 0.5 is midday on day 1*/
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Clouds|Noise", meta = (EditCondition = "bSimulVersion4_2", ClampMin = "8", ClampMax = "64"))
...
/** A multiplier for brightness of the trueSKY environment, 1.0 by default.*/
	...
	UPROPERTY(AdvancedDisplay, EditAnywhere, BlueprintReadWrite, Category = Lighting, meta = (EditCondition = "(bSimulVersion4_2&&(SIMUL_ALTITUDE_DIRECTIONAL_LIGHT))"))
	bool AltitudeLighting;

	/** The render texture to fill in with the cloud shadow.*/
	UPROPERTY(AdvancedDisplay, EditAnywhere, Category = Lighting, BlueprintReadWrite)
	...
	UPROPERTY(EditAnywhere, interp, BlueprintReadWrite, Category=Atmospherics, meta = (EditCondition = "bSimulVersion4_1", ClampMin = "0.0", ClampMax = "20.0"))
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category=Shadows, meta = (ClampMin = "32", ClampMax = "4096"))
	...
	//Returns true if the light was updated. Due to GPU latency, the update will not occur for the first few frames.*/
	////UFUNCTION(BlueprintCallable, Category = TrueSky)
	//static bool UpdateLight(ADirectionalLight* DirectionalLight, float multiplier=1.0f, bool include_shadow=false, bool apply_rotation=true);

	///** Update a single light with either sunlight or moonlight.
	//Updates the colour and intensity of the light. This depends on the time of day, and also the altitude of the DirectionalLight.
	//Returns true if the light was updated. Due to GPU latency, the update will not occur for the first few frames.*/
	////UFUNCTION(BlueprintCallable, Category = TrueSky)
	//static bool UpdateLight2(ADirectionalLight* DirectionalLight, float sun_multiplier=1.0f, float moon_multiplier=1.0f, bool include_shadow=false, bool apply_rotation=true);

	/** Update a single light with either sunlight or moonlight.
	Updates the colour and intensity of the light. This depends on the time of day, and also the altitude of the DirectionalLight.
	Returns true if the light was updated. Due to GPU latency, the update will not occur for the first few frames.*/
	//UFUNCTION(BlueprintCallable, Category = TrueSky)
	static bool UpdateLight(ADirectionalLight* DirectionalLight, float sun_multiplier = 1.0f, float moon_multiplier = 1.0f, bool include_shadow = false, bool apply_rotation = true,bool light_in_space=false);
	...
	Returns true if the light was updated. Due to GPU latency, the update will not occur for the first few frames.*/
	//UFUNCTION(BlueprintCallable, Category = TrueSky)
	static bool UpdateLightComponent(UDirectionalLightComponent *dlc, float sun_multiplier=1.0f, float moon_multiplier=1.0f, bool include_shadow=false, bool apply_rotation=true,bool light_in_space=false);
	...
	UFUNCTION(BlueprintCallable, Category = "TrueSky|Clouds|Keyframe|UID", DisplayName = "Get Interpolated Cloud Keyframe UID")
	...
	UFUNCTION(BlueprintCallable, Category = "TrueSky|Sky|Keyframe|UID", DisplayName = "Get Interpolated Sky Keyframe UID")
