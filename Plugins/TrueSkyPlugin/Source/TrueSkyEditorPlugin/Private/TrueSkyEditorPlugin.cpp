...
public:
	...
	{
		UI_COMMAND(AddSequence					,"Initialize trueSKY..."		,"Launches a wizard to setup trueSKY in the current level.", EUserInterfaceActionType::Button, FInputChord());
		UI_COMMAND(ToggleShowCelestialDisplay	,"Celestial Display"			,"Toggles the celestial display.", EUserInterfaceActionType::ToggleButton, FInputChord());
		UI_COMMAND(ToggleOnScreenProfiling		,"Profiling"					,"Toggles the Profiling display.", EUserInterfaceActionType::ToggleButton, FInputChord());
		UI_COMMAND(ToggleShowFades				,"Atmospheric Tables"			,"Toggles the atmospheric tables overlay.", EUserInterfaceActionType::ToggleButton, FInputChord());
		UI_COMMAND(ToggleShowCubemaps			,"Cubemaps"						,"Toggles the cubemaps overlay.", EUserInterfaceActionType::ToggleButton, FInputChord());
		UI_COMMAND(ToggleShowWaterTextures		,"Water Textures"				,"Toggles the water textures overlay.", EUserInterfaceActionType::ToggleButton, FInputChord());
		//UI_COMMAND(ToggleShowFlowRays			,"Flow Rays"					,"Toggles the Flow Rays debuf visualiser.", EUserInterfaceActionType::ToggleButton, FInputChord());
		...
		UI_COMMAND(ToggleShowRainOverlay		,"Rain Overlay"					,"Toggles the rain overlay.", EUserInterfaceActionType::ToggleButton, FInputChord());
		UI_COMMAND(TriggerCycleCompositingView	,"Cycle Compositing View"		,"Changes which view is shown on the compositing overlay.", EUserInterfaceActionType::Button, FInputChord());
		UI_COMMAND(ToggleShow3DCloudTextures	,"Show 3D Cloud Textures"		,"Toggles the 3D cloud overlay.", EUserInterfaceActionType::ToggleButton, FInputChord());
		//UI_COMMAND(ToggleShow2DCloudTextures	,"Show 2D Cloud Textures"		,"Toggles the 2D cloud overlay.", EUserInterfaceActionType::ToggleButton, FInputChord());
		...
		UI_COMMAND(TriggerShowDocumentation		,"trueSKY Documentation..."		,"Shows the trueSKY help pages.", EUserInterfaceActionType::Button, FInputChord());
		UI_COMMAND(TriggerExportCloudLayer		,"Export Sky As .FBX"			,"Exports the current sky as a .FBX file.", EUserInterfaceActionType::Button, FInputChord());
	}
public:
	...
	else
	{
		...
		//	EditorInstance.EditorWindow->ReshapeWindow( EditorInstance.EditorWindow->GetPositionInScreen(), FVector2D(800.0f, 600.0f) );
			...
			//	GetWindowRect(EditorInstance.EditorWindowHWND, &ParentRect);
				...
				{
					Variant v;
					v.Vec3 = { SequenceActor->OriginLatitude, SequenceActor->OriginLongitude, SequenceActor->OriginHeading };
...
void FTrueSkyEditorPlugin::OnTrueSkySetup(bool CreateDirectionalLight, ADirectionalLight* DirectionalLight,bool CreateTrueSkyLight,UTrueSkySequenceAsset *Sequence
	,bool DeleteFogs,bool DeleteSkyEffects, bool OveridePostProcessVolume
	,bool AltitudeLightFunction)
{
	...
	if(CreateTrueSkyLight)
	{
		...
		UGameplayStatics::GetAllActorsOfClass(GWorld, ASkyLight::StaticClass(), SkyLights);
		for (auto A : SkyLights)
		{
			A->Destroy();
		}
	...
	if(PostProcessVolume==nullptr)
	{
		...
		PostProcessVolume->GetRootComponent()->Bounds.SphereRadius = KINDA_SMALL_NUMBER; //UE4 Bug requires a radius.
	...
	if(DirectionalLight)
	{
		...
		// 50km
		DirectionalLight->GetLightComponent()->SetLightFunctionFadeDistance(100.0f*1000.0f*50.0f);
	}
	if(DeleteFogs)
	{
	...
	if(DeleteSkyEffects)
	{
		...
		UGameplayStatics::GetAllActorsOfClass(GWorld, ASkyAtmosphere::StaticClass(), SkyAtmospheres);
		for (auto A : SkyAtmospheres)
		{
			A->Destroy();
		}
	}
...
void FTrueSkyEditorPlugin::OpenEditor(UTrueSkySequenceAsset*const TrueSkySequence)
{
	...
	if ( EditorInstance == nullptr )
	{
		...
		if ( EditorInstance )
		{
			...
			if(TrueSkySequence->SequenceText.Num() == 0)
				EditorInstance->SaveSequenceData();
