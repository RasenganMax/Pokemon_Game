...
void STrueSkySetupTool::Construct( const FArguments& InArgs )
{
	...
	//UGameplayStatics::GetAllActorsOfClass(GWorld, APostProcessVolume::StaticClass(), PostProcessVolumes); //We can check after

	UClass* BP_Sky_Sphere = FindObject<UClass>(ANY_PACKAGE, TEXT("BP_Sky_Sphere"));

	TArray<AActor*> BP_Sky_Spheres;
	UGameplayStatics::GetAllActorsOfClass(GWorld, BP_Sky_Sphere, BP_Sky_Spheres);
			...
			[
				...
				[
					...
					[
						...
						.ToolTipText(LOCTEXT("TrueSkySetupDeleteFogs", "trueSKY produces its own fog effects, and it is not recommended to use Unreal's Atmospheric and Height fog with trueSKY"))
						[
							SNew(STextBlock)
					...
					[
						...
						.ToolTipText(LOCTEXT("TrueSkySetupDeleteBPSkySphere", "trueSKY will be hidden behind the SkySphere if it is not deleted"))
						[
							SNew(STextBlock)
							.Text( LOCTEXT( "DeleteBP_Sky_Sphere", "Delete BP_Sky_Sphere" ) )
						]
					]
					+ SVerticalBox::Slot()
					.AutoHeight()
					...
					[
						...
						.ToolTipText(LOCTEXT("TrueSkySetupCreateTrueSkyLight", "A TrueSKYLight provides real-time ambient lighting."))
						[
							SNew(STextBlock)
							.Text(LOCTEXT("CreateTrueSkyLight", "Create a trueSKYLight"))
						]
					]
